...
router.delete(
  ...
  async (req: AuthRequest, res: Response, next: NextFunction) => {
    try {
      ...
      if (!item)
        return res.status(404).json({ error: "시간표 항목을 찾을 수 없습니다" });

      const auth = await requireHomeroom(req.user!.id, item.grade, item.classNum);
      if (!auth.ok) return res.status(auth.status!).json({ error: auth.msg });
...
// POST /api/schedule/events (교사 전용)
  ...
  async (req: AuthRequest, res: Response, next: NextFunction) => {
    try {
      if (req.user!.role !== "teacher")
        return res.status(403).json({ error: "선생님만 학교 일정을 추가할 수 있습니다" });
...
// DELETE /api/schedule/events/:id (교사 전용)
router.delete(
  "/events/:id",
  requireAuth,
  async (req: AuthRequest, res: Response, next: NextFunction) => {
    try {
      if (req.user!.role !== "teacher")
        return res.status(403).json({ error: "선생님만 학교 일정을 삭제할 수 있습니다" });
...
// GET /api/schedule/personal-events
router.get(
  "/personal-events",
  requireAuth,
  async (req: AuthRequest, res: Response, next: NextFunction) => {
...
// POST /api/schedule/personal-events
...
// DELETE /api/schedule/personal-events/:id
router.delete(
  "/personal-events/:id",
  requireAuth,
  async (req: AuthRequest, res: Response, next: NextFunction) => {
